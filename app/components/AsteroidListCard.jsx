...
const AsteroidDisplay = ({item}) => (
    ...
    <Text>Velocity: {item.close_approach_data[0].relative_velocity.miles_per_hour}mph</Text>
    <Text>{item.is_potentially_hazardous_asteroid ? 
    "This asteroid is potentially hazardous!" 
    : "This asteroid is not hazardous"}</Text>
    <Text>Miss distance: {item.close_approach_data[0].miss_distance.miles} miles</Text>
    </>
  )
